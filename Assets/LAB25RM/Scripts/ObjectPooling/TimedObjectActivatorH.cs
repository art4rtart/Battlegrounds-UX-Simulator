...
{
    public class TimedObjectActivatorH : MonoBehaviour
    {
        public enum Action
        {
            Activate,
...
{
#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof (TimedObjectActivatorH.Entries))]
    public class EntriesDrawerH : PropertyDrawer
    {
            ...
            if (entries.arraySize > 0)
            {
                ...
                for (int i = 0; i < entries.arraySize; ++i)
                {
                    ...
                    if (entry.FindPropertyRelative("action").enumValueIndex !=
                        (int) TimedObjectActivatorH.Action.ReloadLevel)
                    {
